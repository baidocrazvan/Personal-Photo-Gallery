...
// Hide page title when collapsed navbar is opened
const collapseButton = document.querySelector('button.navbar-toggler');
const navHeading = document.querySelector('h1.navbar-title');
collapseButton.addEventListener('click', () => {
    navHeading.classList.toggle('no-show');
})
...
const toggleButton = document.getElementById('full-view-toggle');
const photoItems = document.getElementsByClassName('item');

// Toggle full animation
if (toggleButton) {
    toggleButton.addEventListener('click', () => {
    const computedStyle = window.getComputedStyle(photoItems[0]);
    const isHidden = computedStyle.backfaceVisibility === 'hidden';
